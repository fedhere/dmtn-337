...
\setDocAbstract{%
This document summarizes the status of the reliability Machine Learning Reliability model (also known as "\texttt{Real}/\texttt{Bogus}") for LSST alerts stream. The model associates to each Difference Image Analysis (DIA) detection a $\rel\in[0-1]$ score designed to measure the likelihood of the veracity of the detection, \emph{i.e.}, astrophysical source (1) \emph{vs} artifact (0). This manuscript documents the architecture and performance of each version of the model, including specific performance characteristics, identified weaknesses and failure modes, mitigation strategies, and details of the training set.
}
